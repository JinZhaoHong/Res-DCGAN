...
def display_maplot_img(images,image_path,save_fig, n = 8, IMAGE_SIZE = 32):
    ...
    for i in range(n):
        for j in range(n):
            ...
            canvas[i * IMAGE_SIZE:(i + 1) * IMAGE_SIZE, j * IMAGE_SIZE:(j + 1) * IMAGE_SIZE, :] = pic
    plt.figure(figsize=(n, n))
    plt.imshow(canvas)
    if (save_fig):
        plt.imsave(image_path, canvas)
    plt.show()
